...
Pesan user: " . $request->input('message');
    ...
    public function wa_api(Request $request)
    {
        $apikey = $request->header('X-API-KEY');
        if ($apikey != env('API_KEY_WA_AI')){
            return response()->json([
                'action' => 'answer_only',
                'message' => 'Api Key Not found'
            ]);
        };
        $raw = $request->input('number_telp');
        $user = User::where('no_telp', $raw)->first();
        if (!$user) {
            return response()->json([
                'action' => 'answer_only',
                'message' => 'Nomor kamu belum terdaftar di aplikasi SchedU!'
            ]);
        }
