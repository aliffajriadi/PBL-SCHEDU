<x-layout-staff title="Manage Account" role="staff" :user="$user">
        ...
        <div>
            <div class="bg-white p-6 rounded-xl shadow-md">
                ...
                <form id="create-form" class="space-y-4">
                    ...
                    <input id="add_name" type="text" name="name" placeholder="Name"
                        class="w-full px-4 py-2 border rounded" required />
                    <input id="add_email" type="email" name="email" placeholder="Email"
                        class="w-full px-4 py-2 border rounded" required />
                    <input id="add_password" type="password" name="password" placeholder="Password"
                        class="w-full px-4 py-2 border rounded" required />
                    <input id="add_address" type="text" name="address" placeholder="Address"
                        class="w-full px-4 py-2 border rounded" required />
                    <div>
                        <label class="block mb-1">Birth Date</label>
                        <input id="add_birth_date" type="date" name="birth_date" class="w-full px-4 py-2 border rounded"
                            required />
                    </div>
                    ...
                    <p id="SuccessInformation" class="font-semibold text-white bg-emerald-400 my-2 p-2 hidden rounded">
                        Create Account Success !!!</p>
                    <button type="button" onclick="insert_data()"
                        class="bg-emerald-400 text-white px-6 py-2 rounded hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        Create Account
                    </button>
                </form>
            </div>
            ...
            <div
                class="mt-6 bg-white p-6 border-dashed border-2 border-gray-300 rounded-xl flex flex-col items-center justify-center text-center h-40">
                <span class="material-icons text-4xl text-gray-400">upload_file</span>
                <p class="text-sm text-gray-500">Drag and Drop file <br><span
                        class="text-emerald-600 font-medium">or</span></p>
                <button
                    class="mt-2 px-4 py-1 border border-emerald-600 text-emerald-600 rounded hover:bg-emerald-100 focus:outline-none focus:ring-2 focus:ring-emerald-500">Browse</button>
            </div>
        </div>
    ...
    <script>
        ...
        async function insert_data() {
            ...
            const response = await fetch('/staff/user', {
                method: 'POST',
                headers: {
            ...
            const successInfo = document.getElementById('SuccessInformation');
            const responseData = await response.json();

            // TAMPILKAN MODAL / NOTIFIKASI
            successInfo.classList.remove('hidden'); // <--- tambahkan ini

            if (responseData.status === true) {
                successInfo.classList.remove('bg-red-500');
                successInfo.classList.add('bg-emerald-400');
                successInfo.innerHTML = responseData.message || 'Data berhasil ditambahkan.';
                ...
                document.getElementById('add_gender').selectedIndex = 0;
                document.getElementById('add_role').selectedIndex = 0;
            } else {
                successInfo.classList.remove('bg-emerald-400');
                successInfo.classList.add('bg-red-500');
                successInfo.innerHTML = responseData.message || 'Gagal menambahkan data.';
            }
            setTimeout(() => {
