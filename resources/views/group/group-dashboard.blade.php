<x-layout title="Group Dashboard" role="{{ $role }}" :user="$user" :image="$userData->profile_pic !== null ? asset('storage/' . $userData->instance->folder_name . '/' . $userData->profile_pic) : 'image/Ryan-Gosling.jpg'">
    <x-nav-group type="name" page="dashboard" group_name="{{ $group->name }}" />
                ...
                <div class="flex flex-col gap-3 max-h-96 overflow-y-auto">
                    @foreach ($members as $member)
                        <div class="flex items-center gap-3 bg-emerald-50 rounded-2xl p-3">
                            <img src="{{ asset($member->user->profile_pic != null ? 'storage/' . $group->instance->folder_name . '/' . $member->user->profile_pic : 'image/Ryan-Gosling.jpg') }}"
                                alt="Member Avatar"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-500">
                                    {{ $member->user->is_teacher == 1 ? "Teacher" : "Student" }}
                                </p>
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="bg-white p-4 shadow-md rounded-2xl h-full">
                ...
                <div class="flex flex-col gap-3 max-h-96 overflow-y-auto">
                    @forelse ($tasks as $task)
                        <div
                            class="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-emerald-50 transition-all duration-300">
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="w-6 h-6 stroke-emerald-400">
            ...
            <div class="bg-emerald-400 p-4 shadow-md rounded-2xl h-full text-white">
                ...
                <div class="flex flex-col gap-3 max-h-96 overflow-y-auto">
                    @forelse ($notes as $note)
                        <div
                            class="p-3 bg-gray-50 text-gray-800 rounded-xl hover:bg-emerald-50 transition-all duration-300">
                            <div class="flex items-center gap-3">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round" class="lucide lucide-notepad-text w-8 h-8 stroke-emerald-400">
                                    <path d="M8 2v4" />
                                    <path d="M12 2v4" />
                                    <path d="M16 2v4" />
